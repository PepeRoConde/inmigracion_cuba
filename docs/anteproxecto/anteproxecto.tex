...
generando archivos master que se conservarán localmente. Quedando a futuro la
decisión del tipo de escáner a utilizar, es necesario contemplar si es de interés
para el público o las instituciones el tener acceso a escaneos de alta resolución
(más de 600 DPI).

\textbf{2. Preprocesado.} Cada imagen será procesada automáticamente para corregir 
perspectiva, eliminar ruido y normalizar la calidad antes de su análisis. 

\textbf{3. Reconocimiento de texto manuscrito (HTR).} Se empleará un sistema de
...
registro. Se probarán los motores de extracción de caracteres \textit{open
source} actuales, y en caso de no obtener unos resultados óptimos, no se
descarta la posibilidad de aplicar un ajuste fino con los propios datos escaneados.

\textbf{4. Estructuración y base de datos.} La información extraída se
...
\textbf{5. Plataforma web.} Se desarrollará una interfaz pública de consulta que permitirá 
...
Por último, quedando aún por decidir el método de digitalización, puede
ser necesario un escáner de formato A3 de alta resolución (más de 600 DPI).
En tal caso será necesario el uso de un escáner tipo \textit{flatbed} o de V,
que respete los lomos de los registros. Un equipo de tales características
puede rondar los 600€.
